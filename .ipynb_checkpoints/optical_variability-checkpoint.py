...
optics_df['diffraction-rho'] = np.degrees(rho_l.astype('float') )
optics_df['geometric-rho'] = np.degrees(rho_r.astype('float') )
...
# +
_x = optics_df['cone length (from the tip)'] + optics_df['lense thickness']
...
plt.show()
if save_figs:
    fig.savefig('/mnt/home/nchua/Dropbox/lamina_figures/focal_rhabdom_scatter_limscale.svg')
    fig.savefig('/mnt/home/nchua/Dropbox/lamina_figures/focal_rhabdom_scatter_limscale.png')

display(results.summary())
# -

fig, ax = plt.subplots(1)
hexplot(node_data={k: {'colour': v,
                      'label': ''} for k, v in zip(optics_df.index, c)}, ax=ax)
plt.show()
...
# +
X = optics_df['cone length (from the tip)'] + optics_df['lense thickness']
X = sm.add_constant(X)
Y = optics_df['f-image']
...
# +
X = optics_df['cone length (from the tip)']
X = sm.add_constant(X)
Y = optics_df['f-image']
...
# +
fig, ax = plt.subplots(8, 2, figsize=(24, 77))
axes = ax.flatten()
